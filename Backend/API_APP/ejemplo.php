...
{
  $query = "CALL sp_obtener_categoria($id)";
...
function handleUpdateCategorias($database)
{
  try {
    ...
    $query = "CALL sp_actualizar_categoria('$Id_Categoria', '$Nombre', '$CreatedAt', '$UpdatedAt')";
    $result = $database->query($query);
    if (!$result) {
      http_response_code(404);
      ...
      if ($affectedRows > 0) {
        ...
        echo json_encode(array("message" => "No se han encontrado filas para actualizar"));
      }
    }
  } catch (Exception $e) {
    http_response_code(403);
...
function handleDeleteCategorias($database)
{
  try {
    $data = json_decode(file_get_contents("php://input"), true);
    $Id_Categoria = $data["Id_Categoria"];
    $query = "CALL sp_eliminar_categoria('$Id_Categoria')";
    $result = $database->query($query);
    if (!$result) {
      http_response_code(404);
      echo json_encode(["controller" => "categorias", "message" => mysqli_error($database)]);
...
function handlePostCategorias($database)
{
  try {
    ...
    if (!$result) {
      ...
      if ($affectedRows > 0) {
        http_response_code(201);
        echo json_encode(array("message" => "Se han insertado " . $affectedRows . " fila(s)"));
      } else {
        http_response_code(204);
        echo json_encode(array("message" => "No se han encontrado filas para insertar"));
      }
    }
  } catch (Exception $e) {
    http_response_code(403);
